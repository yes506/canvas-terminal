...
다이어그램을 그리고 업로드를 클릭하면, 터미널에서 실행 중인 AI CLI 도구가 그림을 봅니다. AI에게 응답을 요청하면 결과가 이미지, Markdown, HTML, SVG, 일반 텍스트 형태로 다시 캔버스에 렌더링됩니다. 여기에 Collaborator 패널을 열어 여러 에이전트 터미널을 띄우고, 공유 태스크와 메모리 파일을 기준으로 협업시킬 수도 있습니다. Canvas Terminal은 시각적 아이디어를 멀티 에이전트 AI 작업 공간으로 바꿔줍니다 — 복사-붙여넣기도, 파일 관리도 필요 없습니다.
...
4. **다운로드** — AI의 응답(Markdown, SVG, HTML, 이미지, 일반 텍스트)이 캔버스에 렌더링되므로, 캔버스는 이미지뿐 아니라 문서형 출력도 바로 다룰 수 있습니다.
