...
**그림이 곧 AI 프롬프트가 되는 터미널.**

다이어그램을 그리고, 업로드를 클릭하면, 터미널에서 실행 중인 AI CLI 도구가 그림을 봅니다. AI에게 응답을 요청하면 결과가 캔버스에 렌더링됩니다. Canvas Terminal은 시각적 아이디어를 AI 대화로 바꿔줍니다 — 복사-붙여넣기도, 파일 관리도 필요 없습니다.
...
- **GUI 인증 대화상자 차단** — git/SSH 프롬프트를 터미널로 강제하여 Tauri 환경에서의 행 방지
