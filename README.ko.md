...
**드로잉, 문서, 여러 AI CLI가 하나의 작업 공간에서 연결되는 캔버스 중심 터미널.**

다이어그램을 그리고 업로드를 클릭하면, 터미널에서 실행 중인 AI CLI 도구가 그림을 봅니다. AI에게 응답을 요청하면 결과가 이미지로 캔버스에 렌더링됩니다 — 지원되는 응답 포맷은 PNG/JPEG, SVG, HTML, Markdown, 일반 텍스트입니다. 이미지나 `.md` 파일은 캔버스에 직접 드롭해서 같은 방식으로 삽입할 수 있고, PDF/DOCX/XLSX/CSV/HWP 파일은 툴바의 Insert File 버튼으로 가져옵니다. Collaborator 패널을 열어 여러 에이전트 터미널을 띄우고, 공유 태스크와 메모리 파일을 기준으로 협업시킬 수도 있습니다. Canvas Terminal은 시각적 아이디어를 멀티 에이전트 AI 작업 공간으로 바꿔줍니다 — 복사-붙여넣기도, 파일 관리도 필요 없습니다.
...
Canvas Terminal을 다른 모든 터미널 에뮬레이터와 차별화하는 핵심 기능입니다. 캔버스는 드로잉을 AI 도구로 보내고, 이미지뿐 아니라 문서형 출력까지 다시 작업 공간으로 렌더링할 수 있습니다.
