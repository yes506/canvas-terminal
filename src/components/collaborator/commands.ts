...
export function getHelpText(): string {
  return [
    ...
    "",
    "Commands:",
    "  @<agent> <msg>    Send message to specific agent (prefix form)",
    "  <msg> @<agent>    Inline mention. Single-agent panes route directly;",
    "                    multi-agent panes open the target selector with that",
    "                    agent prechecked so you can confirm or add recipients.",
    "  @all <msg>        Broadcast to all agents",
    "  <bare text>       Shows target selector before sending",
    "  /status           Show running agents",
    "  /help             Show help",
    "  /fsd-cancel       Cancel any active Auto-Pilot run for the focused leader",
    "",
    "Auto-Pilot: toggle the switch in an agent's header. ON = leader",
    "  orchestrates a pool of headless assistants (Phase 1: single-CLI Pilot).",
    "  Tier picker for x1/x2/x3 returns in Phase 2.",
    "",
    ...
    "",
    "Input keys: Enter = submit / confirm selector. Shift+Enter = newline",
    "  inside the input. Tab = autocomplete @-mention or @file-path.",
  ].join("\n");
}
