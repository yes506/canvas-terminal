...
- 인라인 렌더 대상 포맷 audit: 현재 disposition은 `text/*`만 인라인(`.md`=text/markdown, `.csv`=text/csv는 인라인이라 깨질 수 있음; `.json`=application/json은 attachment/다운로드라 in-browser mojibake 없음; `.log`은 경계 사례). 인라인 렌더되어 한글이 깨지는 포맷을 식별 후 커버 범위를 확정한다
...
- CP949는 EUC-KR의 상위집합이며 샘플이 양쪽에서 동일하게 디코드됨(검증) → 단일 CP949 디코더로 충분하고 EUC-KR 별도 처리는 불필요하다 [resolved]
...
- 기존 ASCII/영문 텍스트 및 비텍스트(이미지/PDF/octet-stream attachment) 동작을 회귀시키지 않을 것
- 선례: `dashboard/server.rs`는 이미 text/plain·json·html 응답에 `charset=utf-8`을 부여한다 — localfile 경로만 예외다
...
- 검증 경로 존재: UTF-8 한글 fixture + CP949 한글 fixture("여섯 살 적에 나는") + ASCII 불변 케이스에 대한 단위/통합(또는 수동) 확인 경로가 있다
...
- UTF-8 파일을 레거시로 오판(또는 그 반대)하여 새로 깨뜨림 — 잘못된 인코딩 감지 자체가 새 버그다 (UTF-8은 자기검증적이므로 'strict UTF-8 우선, 실패 시 CP949' 순서가 안전한 것으로 검증됨)
...
- 감지 순서: strict UTF-8 우선 → 실패 시 CP949 (UTF-8 자기검증성으로 오판 위험 낮음). 안전한 기본 전략으로 보이나 최종은 구현 결정.
